        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div>
            <label className="block text-[9px] tracking-[0.2em] uppercase text-warm-gray mb-4">
              Product Images {existingImages.length + newImagePreviews.length > 0 && `(${existingImages.length + newImagePreviews.length})`}
            </label>
                ...
                {existingImages.map((imgUrl, i) => {
                  ...
                  return (
                    <div key={`existing-${i}`} className="relative group aspect-[3/4] bg-beige overflow-hidden">
                      {src ? <img src={src} alt={`Image ${i + 1}`} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-beige flex items-center justify-center"><span className="text-xs text-warm-gray">No img</span></div>}
                      ...
                      <button type="button" onClick={() => removeExistingImage(i)}
                        className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                    </div>
                  );
                })}
